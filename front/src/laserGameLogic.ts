...
class Game {
    ...
    private lastWebDataSpawnTime: number = 0;
    private isSoundOn: boolean = true;
    private muteBtn!: HTMLElement;

    constructor() {
        this.container = document.getElementById('game-container') as HTMLElement;
        this.scoreEl = document.getElementById('score-val') as HTMLElement;
        this.hpEl = document.getElementById('hp-val') as HTMLElement;
        this.difficultyEl = document.getElementById('difficulty-val') as HTMLElement;
    ...
    private playSound(id: string, volume: number = 0.2) {
        ...
        if (sourceSound) {
            const soundClone = sourceSound.cloneNode(true) as HTMLAudioElement;
    ...
    private updateDifficulty() {
        ...
        if(this.difficultyFactor > 2.0) { this.difficultyEl.style.color = "red"; this.difficultyEl.classList.add('blink'); }
    }
    ...
    private collectPowerUp(p: PowerUpObject) {
        ...
        info.style.position = 'fixed'; info.style.top = '30%'; info.style.width = '100%'; info.style.textAlign='center';
        info.style.color = p.type === WeaponMode.SPREAD ? 'var(--neon-blue)' : 'var(--neon-yellow)';
        info.style.fontSize = '30px'; info.style.fontWeight='bold'; info.style.zIndex = '3000';
        ...
        this.powerUpTimeout = setTimeout(() => {
            ...
            endMsg.style.position = 'fixed'; endMsg.style.top = '30%'; endMsg.style.width = '100%'; endMsg.style.textAlign='center';
            endMsg.style.color = 'white'; endMsg.style.fontSize = '20px'; endMsg.style.zIndex = '3000';
    ...
    private updatePlayer() {
        const dx = this.mouse.x - this.player.pos.x; const dy = this.mouse.y - this.player.pos.y;
        this.rotation = Math.atan2(dy, dx) + Math.PI / 2;
        const accel = { x: 0, y: 0 };
        if (this.keys['KeyW'] || this.keys['ArrowUp']) accel.y -= 1;
        if (this.keys['KeyS'] || this.keys['ArrowDown']) accel.y += 1;
        ...
        if (len > 0) { accel.x /= len; accel.y /= len; }
        this.player.vel.x += accel.x * CONFIG.playerSpeed; this.player.vel.y += accel.y * CONFIG.playerSpeed;
        this.player.vel.x *= CONFIG.friction; this.player.vel.y *= CONFIG.friction;
        this.player.pos.x += this.player.vel.x; this.player.pos.y += this.player.vel.y;
    ...
    private createSingleBullet(angle: number, color: string, offset?: Vector2) {
        const el = document.createElement('div');
        el.className = 'bullet';
    ...
    private spawnEnemy() {
        ...
        const el = document.createElement('div'); el.className = 'enemy'; this.container.appendChild(el);
        const pos = this.getRandomEdgePos();
        this.enemies.push({
    ...
    private getRandomEdgePos(): Vector2 {
        const side = Math.floor(Math.random() * 4);
        if(side === 0) return { x: Math.random() * window.innerWidth, y: -50 };
        if(side === 1) return { x: window.innerWidth + 50, y: Math.random() * window.innerHeight };
        if(side === 2) return { x: Math.random() * window.innerWidth, y: window.innerHeight + 50 };
    ...
    private updateEnemies() {
        ...
        this.enemies.forEach(en => {
            ...
            en.vel.x = Math.cos(angle) * currentSpeed; en.vel.y = Math.sin(angle) * currentSpeed;
            en.pos.x += en.vel.x; en.pos.y += en.vel.y;
            if (en.lastShot !== undefined && now - en.lastShot > shootRate) { this.spawnEnemyBullet(en); en.lastShot = now; }
            if (this.checkOverlap(en, this.player)) { this.takeDamage(15); this.removeEntity(en, this.enemies); }
        });
    }

    private spawnEnemyBullet(enemy: GameObject) {
        const el = document.createElement('div'); el.className = 'enemy-bullet'; this.container.appendChild(el);
    ...
    private updateEnemyBullets() {
        for (let i = this.enemyBullets.length - 1; i >= 0; i--) {
            const b = this.enemyBullets[i]; b.pos.x += b.vel.x; b.pos.y += b.vel.y;
            if (this.isOffscreen(b)) this.removeEntity(b, this.enemyBullets);
        }
    }
    ...
    private updateFloatingEntities(list: GameObject[]) {
        list.forEach(t => {
            ...
            if(t.pos.x < -100) t.pos.x = window.innerWidth + 100;
            else if(t.pos.x > window.innerWidth + 100) t.pos.x = -100;
            if(t.pos.y < -100) t.pos.y = window.innerHeight + 100;
            else if(t.pos.y > window.innerHeight + 100) t.pos.y = -100;
        });
    }

    private updateBullets() {
        for (let i = this.bullets.length - 1; i >= 0; i--) {
            const b = this.bullets[i]; b.pos.x += b.vel.x; b.pos.y += b.vel.y;
            if (this.isOffscreen(b)) this.removeEntity(b, this.bullets);
        }
    }
    ...
    private removeEntity(obj: GameObject, list: GameObject[]) {
        obj.active = false; obj.element.remove();
        const idx = list.indexOf(obj); if(idx > -1) list.splice(idx, 1);
    }
    ...
    private saveAndLoadScores(): HighScore[] {
