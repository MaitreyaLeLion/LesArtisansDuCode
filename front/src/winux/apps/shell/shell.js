...
class VirtualFileSystem {
    ...
    rebuildParentLinks(node, parent) {
        node.parent = parent;
    ...
    resolveNode(pathStr) {
        ...
        if (pathStr.startsWith('/'))
            ...
            currentNode = this.getNodeAt(this.currentPath);
    ...
    getCompletionCandidates(partialPath) {
        let dirPath = '';
        let searchPrefix = '';
        ...
        if (!parentNode || parentNode.type !== 'dir')
            return { matches: [], dirPath, searchPrefix };
        ...
        const formattedCandidates = candidates.map(name => {
            if (parentNode.children[name].type === 'dir')
                return name + '/';
            return name;
        });
...
class Shell {
    ...
    cmdLs(args) {
        if (args.includes('-h') || args.includes('help'))
            return { output: HELP_MESSAGES['ls'] };
        ...
        if (node.type === 'file')
            return { output: `<span class="type-file">${targetPath}</span>` };
    ...
    cmdCd(args) {
        if (args.includes('-h') || args.includes('help'))
            return { output: HELP_MESSAGES['cd'] };
        const cleanArgs = args.filter(a => !a.startsWith('-'));
        if (!cleanArgs[0])
            return { output: '' };
        const target = this.fs.resolveNode(cleanArgs[0]);
        if (!target)
            return { output: `cd: ${cleanArgs[0]}: No such file or directory`, error: true };
        if (target.type !== 'dir')
            return { output: `cd: ${cleanArgs[0]}: Not a directory`, error: true };
        ...
        for (const p of parts) {
            if (p === '.')
                continue;
            if (p === '..') {
                if (this.fs.currentPath.length > 0)
                    this.fs.currentPath.pop();
            }
            else {
    ...
    cmdTouch(args) {
        if (args.includes('-h') || args.includes('help'))
            return { output: HELP_MESSAGES['touch'] };
        const cleanArgs = args.filter(a => !a.startsWith('-'));
        if (!cleanArgs[0])
    ...
    cmdCat(args, stdin) {
        if (args.includes('-h') || args.includes('help'))
            return { output: HELP_MESSAGES['cat'] };
        const cleanArgs = args.filter(a => !a.startsWith('-'));
        if (cleanArgs.length === 0)
            return { output: this.escapeHtml(stdin) };
        const node = this.fs.resolveNode(cleanArgs[0]);
        if (!node)
            return { output: `cat: ${cleanArgs[0]}: No such file`, error: true };
        if (node.type === 'dir')
            return { output: `cat: ${cleanArgs[0]}: Is a directory`, error: true };
        return { output: this.escapeHtml(node.content) };
    }
    cmdEcho(args) {
        if (args.includes('-h') || args.includes('help'))
            return { output: HELP_MESSAGES['echo'] };
        let text = args.join(' ');
    ...
    cmdRm(args) {
        if (args.includes('-h') || args.includes('help'))
            return { output: HELP_MESSAGES['rm'] };
        const cleanArgs = args.filter(a => !a.startsWith('-'));
        if (!cleanArgs[0])
            return { output: 'rm: missing operand', error: true };
        const cwd = this.fs.getCwdNode();
        const targetName = cleanArgs[0];
        if (!cwd.children[targetName])
            return { output: `rm: '${targetName}': No such file`, error: true };
    ...
    cmdGrep(args, stdin) {
        if (args.includes('-h') || args.includes('help'))
            return { output: HELP_MESSAGES['grep'] };
        const cleanArgs = args.filter(a => !a.startsWith('-'));
        if (cleanArgs.length === 0)
            return { output: 'grep: usage: grep [pattern] [file]', error: true };
        ...
        if (cleanArgs.length > 1) {
            ...
            if (!node || node.type !== 'file')
                return { output: `grep: ${cleanArgs[1]}: error`, error: true };
    ...
    cmdColor(args) {
        if (args.includes('-h') || args.includes('help'))
            return { output: HELP_MESSAGES['color'] };
        if (!args[0])
            return { output: 'color: usage: color <name|#hex>', error: true };
    ...
    cmdSudo(args) {
        if (args.includes('-h') || args.includes('help'))
            return { output: HELP_MESSAGES['sudo'] };
        if (args[0] === 'su') {
